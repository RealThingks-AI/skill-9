...
const Approvals = () => {
  ...
  const filteredHistory = groupedHistory.filter(group =>
    group.employeeName.toLowerCase().includes(searchTerm.toLowerCase()) ||
    group.employeeEmail.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <div className="h-screen flex flex-col overflow-hidden">
      ...
      <div className="flex-shrink-0 flex items-center justify-between h-16 px-6 border-b border-sidebar-border">
        <div className="flex items-center gap-3">
          <h1 className="text-2xl tracking-tight text-foreground font-medium">Approvals Dashboard</h1>
              ...
              <div className="space-y-2">
                {filteredHistory.map(employee => (
                  <EmployeeHistoryDetail
                    key={employee.userId}
                    employee={employee}
                    isExpanded={expandedHistoryId === employee.userId}
                    onToggle={() => setExpandedHistoryId(prev => prev === employee.userId ? null : employee.userId)}
                  />
                ))}
              </div>
            )}
